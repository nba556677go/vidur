...
class SklearnExecutionTimePredictor(BaseExecutionTimePredictor):
    ...
    def _get_attention_df_with_derived_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for column in [
            ...
            "time_stats.attn_kv_cache_save_memory_mb.median",
        ]:
            if column not in df_with_derived_features.columns:
                df_with_derived_features[column] = 0
            else:
        ...
        for model_name in model_names:
            if model_name == "attn_kv_cache_save":
                models[model_name] = self._train_model(
                    model_name=model_name,
                    df=attention_df,
                    feature_cols=["num_tokens", "time_stats.attn_kv_cache_save_memory_mb.median"],
                    target_col=f"time_stats.{model_name}.median",
                )
            else:
                models[model_name] = self._train_model(
    ...
    def _predict_for_compute_models(self) -> Dict[str, Any]:
        ...
        # Regular compute models that only use num_tokens
        model_names = [
            "attn_pre_proj",
            "attn_post_proj",
            "mlp_up_proj",
        ...
        # Handle attn_kv_cache_save separately with both features
        X_kv_cache = pd.DataFrame([
            (num_tokens, self._calculate_kv_cache_memory_mb(num_tokens)) 
            for num_tokens in num_token_range
        ], columns=["num_tokens", "time_stats.attn_kv_cache_save_memory_mb.median"])
        
        predictions["attn_kv_cache_save"] = self._get_model_prediction(
            "attn_kv_cache_save", models["attn_kv_cache_save"], X_kv_cache
        )
    ...
    def _calculate_kv_cache_memory_mb(self, num_tokens: int) -> float:
        """Calculate KV cache memory in MB for given number of tokens."""
        head_dim = self._model_config.embedding_dim // self._model_config.num_q_heads
        memory_bytes = (
            num_tokens * 
    ...
    def _get_attention_kv_cache_save_execution_time(
        ...
        # Use both num_tokens and calculated memory_mb
        memory_mb = self._calculate_kv_cache_memory_mb(batch.total_num_tokens)
        return self._predictions["attn_kv_cache_save"][(batch.total_num_tokens, memory_mb)]
